...
void imprimir_mensaje_error_ejecucion(status_exe_t estado){
	switch(estado){
		case EXE_OPCODE_INV:
			fprintf(stderr , "%s : %s \n", EXE_MSJ_ERROR , EXE_MSJ_ERROR_OPCODE_INV);
			break;
		case EXE_MEMORIA_FUERA_RANGO:
			fprintf(stderr , "%s : %s \n", EXE_MSJ_ERROR , EXE_MSJ_ERROR_MEMORIA_RANGO);
			break;
		case EXE_ERROR_ENTRADA:
			fprintf(stderr , "%s : %s \n", EXE_MSJ_ERROR , EXE_MSJ_ERROR_ENTRADA);
			break;
		case EXE_NULL:
			fprintf(stderr, "%s : %s \n", EXE_MSJ_ERROR , EXE_MSJ_ERROR_NULL);
			break;
		case EXE_OK:
			break;
	}
...
bool_t validar_dir_memoria(lista_t * memoria_programa){
	...
	for(i = 0; i < memoria_programa->largo; i++){
		...
		if(nodo_aux->valor == EXE_OPERADOR_HALT){
			return VERDADERO;
		}
...
status_exe_t ejecutar_memoria( struct simpletron * cabeza){
	...
	int opcode;
	status_exe_t estado_exe = EXE_OPCODE_INV;
...
status_exe_t operacion_jmp(int posicion_memoria , struct simpletron * cabeza){
	...
	cabeza->contador = nodo_aux->valor % EXE_DIVISOR;
	return EXE_OK;
}
